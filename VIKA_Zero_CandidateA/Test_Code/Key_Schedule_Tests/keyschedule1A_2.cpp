/*
    This is part of the VIKA_Zero_CandidateA encryption system tests, the source code file for testing the keyschedule 1A
...
struct test_data {
    ...
    unsigned int original_ks_bits[8448 * 8];
    unsigned int flipped_ks[128][528][16];
    unsigned int flipped_ks_bits[128][8448 * 8];
    unsigned int highest_pop_count = 0;
    unsigned int lowest_pop_count = 67584;
    unsigned int pop_count_range;
    double average_pop_count;
    unsigned int highest_difference = 0;
    unsigned int lowest_difference = 67584;
    ...
    for(unsigned int n_shuffles = 0; n_shuffles < 1; n_shuffles++) {
        for(unsigned int n_cycles_per_pbox = 0; n_cycles_per_pbox < 1; n_cycles_per_pbox++) {
            for(unsigned int n_cycles = 0; n_cycles < 1; n_cycles++) {
                ...
                unsigned int highest_pop_count = 0, lowest_pop_count = 67584, highest_difference = 0, lowest_difference = 67584;
                ...
                for(unsigned int n_tests = 0; n_tests < NTEST_KEYS; n_tests++) {
                    ...
                    for(unsigned int n_ks_blocks = 0; n_ks_blocks < 528; n_ks_blocks++) {
                        encrypt_test4D(data_collection->original_ks[n_ks_blocks], temp_sbox, temp_pbox, cycles_per_pbox[n_cycles_per_pbox], cycles[n_cycles], temp_xor_bytes);
                    ...
                    unsigned int temp_ks[8448];
                    n_blocks = 0;
                    for(unsigned int n_ks = 0, n_bytes = 0; n_ks < 8448; n_ks++) {
                    ...
                    bytesToBits(temp_ks, 8448, data_collection->original_ks_bits);
                    ...
                    for(unsigned int n_flipped_keys = 0; n_flipped_keys < 128; n_flipped_keys++) {
                        ...
                        //Encrpyt each block with CBC mode
                        for(unsigned int n_ks_blocks = 0; n_ks_blocks < 528; n_ks_blocks++) {
                            encrypt_test4D(data_collection->flipped_ks[n_flipped_keys][n_ks_blocks], temp_sbox, temp_pbox, cycles_per_pbox[n_cycles_per_pbox], cycles[n_cycles], temp_xor_bytes);
                        ...
                        bytesToBits(temp_ks, 8448, data_collection->flipped_ks_bits[n_flipped_keys]);
                    }
                    ...
                    for(unsigned int n_flipped_keys = 0; n_flipped_keys < 128; n_flipped_keys++) {
                        ...
                        for(unsigned int n_bits = 0; n_bits < 8448 * 8; n_bits++) {
                    ...
                    //FLAG
                    std::ofstream keyschedule_file("keyschedule1Atest50000.txt", std::ios::app);
                    if (keyschedule_file.is_open()) {
                        
                        keyschedule_file << 
                        "Highest Pop Count:\t" << (double)data_collection->highest_pop_count / 67584 * 100 << "%" <<
                        "\nLowest Pop Count:\t" << (double)data_collection->lowest_pop_count / 67584 * 100 << "%" <<
                        "\nPop Count Range:\t" << data_collection->pop_count_range <<
                        "\nAverage Pop Count:\t" << (double)data_collection->average_pop_count / 67584 * 100 << "%" <<
                        "\nHighest Difference:\t" << data_collection->highest_difference << "/67584" <<
                        "\nLowest Difference:\t" << data_collection->lowest_difference << "/67584" <<
                        "\nDifference Range:\t" << data_collection->difference_range <<
                        "\nAverage Difference:\t" << data_collection->average_difference << "/67584" <<
                        "\n\n";
                ...
                //FLAG
                std::ofstream keyschedule_file2("keyschedule1Atest50000_final.txt", std::ios::app);
                if (keyschedule_file2.is_open()) {
                    
                    keyschedule_file2 << 
                    "Final Results:" <<
                    "\nHighest Pop Count:\t" << (double)highest_pop_count / 67584 * 100 << "%" <<
                    "\nLowest Pop Count:\t" << (double)lowest_pop_count / 67584 * 100 << "%" <<
                    "\nAverage Pop Count Range:\t" << average_pop_count_range <<
                    "\nAverage Pop Count:\t" << (double)total_average_pop_count / 67584 * 100 << "%" <<
                    "\nHighest Difference:\t" << highest_difference << "/67584" <<
                    "\nLowest Difference:\t" << lowest_difference << "/67584" <<
                    "\nAverage Difference Range:\t" << average_difference_range <<
                    "\nAverage Difference:\t" << total_average_difference << "/67584" <<
                    "\n\n";
