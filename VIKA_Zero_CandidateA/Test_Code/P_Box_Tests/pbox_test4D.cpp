/*
    This is part of the VIKA_Zero_CandidateA encryption system tests, the source code file for testing the P-Box and encryption setup 4D
...
const unsigned int NTEST_BLOCKS = 1000; //FLAG
const unsigned int NTEST_BOXES = 1000; //1,000 * 2000 = 2,000,000
...
void encrypt_test(unsigned int block[], unsigned int sbox[], unsigned int pbox[], int pbox_cycles, int ncycles, unsigned int xor_bytes[]);
            ...
            for(unsigned int n_cycles = 0; n_cycles < 1; n_cycles++) {
                ...
                for(unsigned int n_boxes = 0; n_boxes < NTEST_BOXES; n_boxes++) {
                    ...
                    for(unsigned int nblock_set = 0; nblock_set < NTEST_BLOCKS; nblock_set++) {
                        ...
                        individual_tests++;
                        system("clear");
                        std::cout << "Test #: " << test_count << "/6\nIndividual Test: " << individual_tests << "/" << NTEST_BLOCKS * NTEST_BOXES << "\n";

                        for(unsigned int n = 0; n < 16; n++) {
                            test_blocks[nblock_set].original_cblock[n] = test_blocks[nblock_set].original_pblock[n];
                        }

                        encrypt_test(test_blocks[nblock_set].original_cblock, sbox, pbox, single_test_data.n_cycles_per_pbox, single_test_data.n_cycles, xor_bytes);
                        ...
                        for(unsigned int nblock_single = 0; nblock_single < 128; nblock_single++) {
                            ...
                            encrypt_test(test_blocks[nblock_set].flipped_cblocks[nblock_single], sbox, pbox, single_test_data.n_cycles_per_pbox, single_test_data.n_cycles, xor_bytes);
                        }
                ...
                //FLAG
                std::ofstream pbox_file("pbox_test4D_1000x1000.txt", std::ios::app);
