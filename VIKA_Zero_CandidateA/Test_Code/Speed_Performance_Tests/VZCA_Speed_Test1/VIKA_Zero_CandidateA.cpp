...
void VIKA_Zero_CandidateA::encrypt(std::string file, std::string mode) {
	...
	if (mode == "ecb") {
        ...
        //End timer
        auto encrypt_stop = high_resolution_clock::now();
        auto duration_encrypt = duration_cast<microseconds>(encrypt_stop - encrypt_start);
        std::cout << "Encrypt time: " << duration_encrypt.count() << " microseconds.\n";
        std::cout << "Time per block: " << duration_encrypt.count() / 6553600 << " microseconds\n";
...
void VIKA_Zero_CandidateA::decrypt(std::string file, std::string mode) {
	...
	if (mode == "ecb") {
        ...
        //End timer
        auto decrypt_stop = high_resolution_clock::now();
        auto duration_decrypt = duration_cast<microseconds>(decrypt_stop - decrypt_start);
        std::cout << "Decrypt time: " << duration_decrypt.count() << " microseconds.\n";
        std::cout << "Time per block: " << duration_decrypt.count() / 6553600 << " microseconds\n";
