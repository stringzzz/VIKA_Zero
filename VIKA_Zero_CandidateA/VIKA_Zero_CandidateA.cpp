...
void VIKA_Zero_CandidateA::key_schedule1A(unsigned char input_key[], unsigned char output_key_schedule[144][16]) {
    //Get shifted blocks
    unsigned char temp_key_schedule[289][16];
    ...
    for(unsigned int n_blocks = 16; n_blocks < 289;) {
    ...
    unsigned char temp_ks_sbox[1024];
    unsigned char temp_ks_pbox[1024];
    ...
    unsigned char temp_ks_sbox_bits[1024 * 8];
    ...
    unsigned char temp_ks_pbox_bits[1024 * 8];
    ...
    for(unsigned int n_ks_blocks = 0; n_ks_blocks < 144; n_ks_blocks++, n_blocks++) {
        for(unsigned int n = 0; n < 16; n++) {
    ...
    for(unsigned int n_ks_blocks = 0; n_ks_blocks < 144; n_ks_blocks++) {
...
void VIKA_Zero_CandidateA::distribute_key_schedule(unsigned char key_schedule[144][16]) {
    //Separate temp KS into pieces for setting up S and P box and XOR for encryption
    unsigned int n_blocks = 0;
    for(unsigned int n_ks = 0, n_bytes = 0; n_ks < 1024; n_ks++) {
    ...
    bytesToBits(this->KS_SBOX, 1024, this->KS_SBOX_BITS);
    ...
    bytesToBits(this->KS_PBOX, 1024, this->KS_PBOX_BITS);
...
void VIKA_Zero_CandidateA::key_generator1B(unsigned char original_key[]) {
        ...
        for(unsigned int nloops = 0; nloops < 4; nloops++) {
            ...
            for(unsigned int nks_bytes = 0, nks_blocks = 0; nks_blocks < 144; nks_bytes++) {
                if (nks_bytes > 15) {
                    nks_bytes = 0;
                    nks_blocks++;
                    if (nks_blocks >= 144) {
