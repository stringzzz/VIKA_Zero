...
/*
	VIKA_Zero_CandidateA ERROR: Vanishing Illusion Key Automator Zero (Candidate A)
...
void VIKA_Zero_CandidateA::key_schedule1A(unsigned char input_key[], unsigned char output_key_schedule[][16]) {
    //Get shifted blocks
    unsigned char temp_key_schedule[1057][16];
    ...
    unsigned char temp_ks_sbox[4096];
    unsigned char temp_ks_pbox[4096];
    ...
    unsigned char temp_ks_sbox_bits[4096 * 8];
    ...
    unsigned char temp_ks_pbox_bits[4096 * 8];
...
void VIKA_Zero_CandidateA::distribute_key_schedule(unsigned char key_schedule[528][16]) {
    //Separate temp KS into pieces for setting up S and P box and XOR for encryption
    unsigned int n_blocks = 0;
    for(unsigned int n_ks = 0, n_bytes = 0; n_ks < 4096; n_ks++) {
    ...
    bytesToBits(this->KS_SBOX, 4096, this->KS_SBOX_BITS);
    ...
    bytesToBits(this->KS_PBOX, 4096, this->KS_PBOX_BITS);
...
void VIKA_Zero_CandidateA::key_generator1B(unsigned char original_key[]) {
    //This is for conveniant test purposes only
    //Don't use it for anything secure... FOOL!
    unsigned char temp_ks[528][16];
    for(unsigned int n = 0; n < 16; n++) {
    ...
    for(unsigned int nloops = 0; nloops < 4; nloops++) {
        ...
        for(unsigned int nks_bytes = 0, nks_blocks = 0; nks_blocks < 528; nks_bytes++) {
            if (nks_bytes > 15) {
                nks_bytes = 0;
                nks_blocks++;
                if (nks_blocks >= 528) {
...
void VIKA_Zero_CandidateA::setKeyS(std::string keyString) {
	...
	if (keyString.length() > 16) {
		throw VIKA_Zero_CandidateA_Exception("VIKA_Zero_CandidateA ERROR: Key as string must be exactly 16 characters, input key is greater then 16 characters.");
	}
	if (keyString.length() < 16) {
		throw VIKA_Zero_CandidateA_Exception("VIKA_Zero_CandidateA ERROR: Key as string must be exactly 16 characters, input key is less then 16 characters.");
	}
...
void VIKA_Zero_CandidateA::setKeyH(std::string hexString) {
	...
	if (hexString.length() > 32) {
		throw VIKA_Zero_CandidateA_Exception("VIKA_Zero_CandidateA ERROR: Key as hex string must be exactly 32 digits, input key is greater than 32 digits.");
	}
	if (hexString.length() < 32) {
		throw VIKA_Zero_CandidateA_Exception("VIKA_Zero_CandidateA ERROR: Key as hex string must be exactly 32 digits, input key is less than 32 digits.");
	}
...
void VIKA_Zero_CandidateA::encrypt(std::string file, std::string mode) {
	...
	if (!keySet) {
		throw VIKA_Zero_CandidateA_Exception("VIKA_Zero_CandidateA ERROR: Key must be set to encrypt.");
	}
	if (mode != "ecb" && mode != "cbc") {
		throw VIKA_Zero_CandidateA_Exception("VIKA_Zero_CandidateA ERROR: Invalid encrypt mode, must be \"ecb\" or \"cbc\".");
	}
	...
	if (fileHandle.is_open()) {
		...
		throw VIKA_Zero_CandidateA_Exception("VIKA_Zero_CandidateA ERROR: Could not open file for read and encrypt.");
	}
	...
	if (file2.is_open()) {
		...
		throw VIKA_Zero_CandidateA_Exception("VIKA_Zero_CandidateA ERROR: Could not open file for write after encrypt.");
	}
...
void VIKA_Zero_CandidateA::decrypt(std::string file, std::string mode) {
	...
	//cbc: Cipher Block Chaining mode
	if (!this->keySet) {
		throw VIKA_Zero_CandidateA_Exception("VIKA_Zero_CandidateA ERROR: Key must be set to decrypt.");
	}
	if (mode != "ecb" && mode != "cbc") {
		throw VIKA_Zero_CandidateA_Exception("VIKA_Zero_CandidateA ERROR: Invalid decrypt mode, must be \"ecb\" or \"cbc\".");
	}
	...
	if (file2.is_open()) {
		...
		throw VIKA_Zero_CandidateA_Exception("VIKA_Zero_CandidateA ERROR: Could not open file for write after decrypt.");
	}
