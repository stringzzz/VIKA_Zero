...
private:
	...
	unsigned char keySchedule[144][16];
	unsigned char Sbox[256];
	unsigned char Pbox[32];
	unsigned char revSbox[256];
	unsigned char revPbox[32];
	unsigned char KS_SBOX[1024];
	unsigned char KS_SBOX_BITS[1024 * 8];
	unsigned char KS_PBOX[1024];
	unsigned char KS_PBOX_BITS[1024 * 8];
    ...
    void key_schedule1A(unsigned char input_key[], unsigned char output_key_schedule[144][16]);
    void distribute_key_schedule(unsigned char key_schedule[144][16]);
    void setup_boxes();
