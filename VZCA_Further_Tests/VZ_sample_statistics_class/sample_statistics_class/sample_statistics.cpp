...
std::vector<double> Sample_Statistics::outliers() {
	if (!outliers_calculated) {
		...
		for(double value : data) {
			if (value >= lower_fence) {
				...
				this->outlier_values.push_back(value);
			}
		}
		for(unsigned int i = this->n - 1; data[i] > upper_fence; i--) {
			if (data[i] > upper_fence) {
				this->outlier_values.push_back(data[i]);
			}
		}
