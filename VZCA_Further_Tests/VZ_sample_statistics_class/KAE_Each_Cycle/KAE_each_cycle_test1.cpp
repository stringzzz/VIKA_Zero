...
const unsigned int NTEST_KEYS = 500; //Takes approx. 35 min //FLAG1
...
struct block {
	...
	unsigned int original_ciphertext[17][16];
...
int main() {
	...
	for(unsigned int n_tests = 0; n_tests < NTEST_KEYS; n_tests++) {
		...
		for(unsigned int nblocks = 0; nblocks < NTEST_PLAINTEXTS; nblocks++) {
			for(unsigned int n = 0; n < 16; n++) {
				plaintext_blocks[nblocks].original_ciphertext[0][n] = plaintext_blocks[nblocks].plaintext[n];
			}
		...
		for(unsigned int nblocks = 0; nblocks < NTEST_PLAINTEXTS; nblocks++) {
			encrypt_test_original(plaintext_blocks[nblocks].original_ciphertext, temp_sbox, temp_pbox, cycles_per_pbox, cycles, temp_xor_bytes);
		}
		...
		for(unsigned int n_flipped_keys = 0; n_flipped_keys < 128; n_flipped_keys++) {
			...
			for(unsigned int nblocks = 0; nblocks < NTEST_PLAINTEXTS; nblocks++) {
				encrypt_test_flipped(plaintext_blocks[nblocks].original_ciphertext, plaintext_blocks[nblocks].flipped_ciphertext[n_flipped_keys], difference_distribution, temp_sbox, temp_pbox, cycles_per_pbox, cycles, temp_xor_bytes);
			}
	...
	//FLAG
	for(unsigned int m = 0; m < 16; m++) {
		std::string filename = "kae_distribution_cycle" + std::to_string(m + 1) + ".txt";
		std::ofstream kae_file(filename, std::ios::out);
		if (kae_file.is_open()) {
			
			for(unsigned int n = 0; n <= 128; n++) {
				kae_file << n << " " << difference_distribution[m][n] << "\n";
			}
