/*
	...
	the source code file for testing the keyschedule 1A Plaintext Avalanche Effect
...
const unsigned int NTEST_KEYS = 1000; //500 * 1000 = 500,000
const unsigned int ks_blocks = 144;
const unsigned int box_bytes = 1024;
const unsigned int inner_ks_blocks = 289;
const std::string output_filename = "PAE_Distribution1.txt";
...
int main(int argc, char *argv[]) {
	...
	for(unsigned int n_keys = 0; n_keys < NTEST_KEYS; n_keys++) {
		...
		for(unsigned int nblock_set = 0; nblock_set < NTEST_BLOCKS; nblock_set++) {
			...
			encrypt_test4D(test_blocks[nblock_set].original_cblock, sbox, pbox, cycles_per_pbox, cycles, xor_bytes);
			...
			for(unsigned int nblock_single = 0; nblock_single < 128; nblock_single++) {
				...
				encrypt_test4D(test_blocks[nblock_set].flipped_cblocks[nblock_single], sbox, pbox, cycles_per_pbox, cycles, xor_bytes);
			}
	...
	std::ofstream keyschedule_file2(output_filename, std::ios::app);
