/*
	...
	the source code file for testing the Plaintext Avalanche Effect
	(With Diffusion Slide 2) FOR EACH cycle of encryption
...
const unsigned int NTEST_BLOCKS = 500; //FLAG1
const unsigned int NTEST_KEYS = 1000; //500 * 1000 = 500,000 (Takes approx 64 min)
...
struct block {
	...
	unsigned int original_cblock[17][16];
...
void encrypt_test_original(unsigned int block[][16], unsigned int sbox[], unsigned int pbox[], int pbox_cycles, int ncycles, unsigned int xor_bytes[]);
...
int main() {
	...
	for(unsigned int n_keys = 0; n_keys < NTEST_KEYS; n_keys++) {
		...
		genBox3(sbox, 256, temp_ks_sbox_bits, n_sbox_shuffles);
		unsigned int pbox[32];
		genBox3(pbox, 32, temp_ks_pbox_bits, n_pbox_shuffles);
		...
		for(unsigned int nblock_set = 0; nblock_set < NTEST_BLOCKS; nblock_set++) {
			...
			for(unsigned int n = 0; n < 16; n++) {
				test_blocks[nblock_set].original_cblock[0][n] = test_blocks[nblock_set].original_pblock[n];
			}

			encrypt_test_original(test_blocks[nblock_set].original_cblock, sbox, pbox, cycles_per_pbox, cycles, xor_bytes);
			...
			for(unsigned int nblock_single = 0; nblock_single < 128; nblock_single++) {
				...
				encrypt_test_flipped(test_blocks[nblock_set].original_cblock, test_blocks[nblock_set].flipped_cblocks[nblock_single], difference_distribution, sbox, pbox, cycles_per_pbox, cycles, xor_bytes);
			}
	...
	//FLAG
	for(unsigned int m = 0; m < 16; m++) {
		std::string filename = "pae_distribution_cycle" + std::to_string(m + 1) + ".txt";
		std::ofstream pae_file(filename, std::ios::out);
		if (pae_file.is_open()) {
			
			for(unsigned int n = 0; n <= 128; n++) {
				pae_file << n << " " << difference_distribution[m][n] << "\n";
			}
...
void encrypt_test_original(unsigned int block[][16], unsigned int sbox[], unsigned int pbox[], int pbox_cycles, int ncycles, unsigned int xor_bytes[]) {
	...
	for(unsigned int cycles = 1; cycles <= ncycles; cycles++) {
		...
		sBox(block[0], 16, sbox);

		if (cycles % pbox_cycles == 0 && cycles % 4 != 0) {
			apply_pbox(block[0], pbox);
		} else if (cycles % 4 == 0) {
			diffusion_slide2_encrypt(block[0]);
		}

		for(unsigned int n = 0; n < 16; n++, nxor_bytes++) {
			block[0][n] ^= xor_bytes[nxor_bytes];
		}

		for(unsigned int i = 0; i < 16; i++) {
			block[cycles][i] = block[0][i];
		}
...
void compare_original_and_flipped(unsigned int original[], unsigned int flipped[], double difference_distribution[]) {
	unsigned int original_bits[128];
	unsigned int flipped_bits[128];
	bytesToBits(original, 16, original_bits);
	bytesToBits(flipped, 16, flipped_bits);
	
	//Get differences between original and flipped ciphertexts
	unsigned int temp_differences = 0;
	for(unsigned int n_bits = 0; n_bits < 128; n_bits++) {
		if (flipped_bits[n_bits] != original_bits[n_bits]) {
			temp_differences++;
		}
	}
