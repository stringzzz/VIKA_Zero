/*
	...
	the source code file for doing analysis on the S-Box & P-Box
...
const unsigned int NTEST_KEYS = 4608000; //Takes approx. 5.6 Hours //FLAG1
...
int main(int argc, char *argv[]) {
	...
	const std::string output_filename2 = argv[2];
	...
	for(unsigned int n_tests = 0; n_tests < NTEST_KEYS; n_tests++) {
		...
		//Track all possible inputs and resulting outputs for the S-Box:
		for(unsigned int i = 0; i < 256; i++) {
			sbox_io[i][temp_sbox[i]]++;
		}

		//Track all possible inputs and resulting outputs for the P-Box:
		for(unsigned int i = 0; i < 32; i++) {
			pbox_io[i][temp_pbox[i]]++;
		}
	...
	//Output P-Box io
	std::ofstream pbox_file(output_filename2, std::ios::out);
	if (pbox_file.is_open()) {
		
		for(unsigned int m = 0; m < 32; m++) {
			for(unsigned int n = 0; n < 32; n++) {
				pbox_file << m << "->" << n << " " << pbox_io[m][n] << "\n";
			}
		}
